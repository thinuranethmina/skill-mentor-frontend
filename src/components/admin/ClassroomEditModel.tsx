...
export function ClassroomEditModel({ classroom, classrooms, setClassrooms }: Props) {
    const axios = useAxiosWithAuth();
    const [open, setOpen] = useState(false);
    const [mentors, setMentors] = useState<Mentor[]>([]);

    const [selectedMentor, setSelectedMentor] = useState<Mentor>({...classroom.mentor, classrooms: []});
    ...
    const { register, handleSubmit, reset, formState: { errors, isValid }, control } = useForm<FormData>({
        ...
        defaultValues: {
            ...
            mentor: classroom.mentor.mentor_id.toString(),
        }
    });
    ...
    const onSubmit = async (data: FieldValues) => {
        ...
        try {
            const res = await axios.put("/academic/classroom", formData, {
                headers: {
            ...
            if (res.status === 200) {
                ...
                setClassrooms(classrooms.map((c) => c.class_room_id === classroom.class_room_id ? newClass : c));
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[750px]">
                <form action="action" onSubmit={handleSubmit(onSubmit)}>
                    <DialogHeader>
                        <DialogTitle>Edit {classroom.title}</DialogTitle>
                    </DialogHeader>
                    <div className="grid lg:grid-cols-2 gap-4 mt-3">
                        ...
                        <div className="grid gap-3">
                            <Label htmlFor="class_image">Class Image (optional)</Label>
